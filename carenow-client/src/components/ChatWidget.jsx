        ...
        <div className={`bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden mb-3 flex flex-col transition-all duration-300 ${isExpanded ? 'w-[90vw] md:w-[700px]' : 'w-[350px]'}`} style={{ height: isExpanded ? '85vh' : '480px', boxShadow: '0 10px 40px -10px rgba(0,0,0,0.2)' }}>
          ...
          <div className="flex-1 overflow-y-auto p-4 bg-gray-50 flex flex-col gap-3">
            {!isAuthenticated ? (
              <div className="h-full flex items-center justify-center">
                <div className="bg-white border border-gray-100 rounded-2xl p-4 text-center max-w-[280px]">
                  <p className="text-sm font-semibold text-gray-800 mb-2">Đăng nhập để dùng chatbot</p>
                  <p className="text-xs text-gray-500 mb-3">Vui lòng đăng nhập để hỏi đáp tình trạng bệnh và nhận hỗ trợ.</p>
                  <Link
                    to="/login"
                    onClick={() => setIsOpen(false)}
                    className="inline-flex items-center justify-center rounded-lg bg-blue-600 text-white text-sm font-semibold px-3 py-2 hover:bg-blue-700"
                  >
                    Đi đến đăng nhập
                  </Link>
                </div>
          ...
          <div className="p-3 bg-white border-t border-gray-100 flex gap-2 items-end">
            <textarea 
              ...
              placeholder={isAuthenticated ? "Nhập tin nhắn..." : "Vui lòng đăng nhập để sử dụng chatbot"}
              disabled={!isAuthenticated}
              className="flex-1 bg-gray-50 border border-gray-200 rounded-2xl px-3.5 py-2 text-[14px] focus:outline-none focus:border-blue-400 focus:bg-white transition-colors resize-none overflow-hidden min-h-[40px] max-h-[100px] disabled:opacity-60 disabled:cursor-not-allowed"
              rows="1"
            />
            <button 
              onClick={sendMessage}
