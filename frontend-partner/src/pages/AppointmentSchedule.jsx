...
export default function AppointmentSchedule() {
  ...
  const [ovSessionType, setOvSessionType] = useState('');
                          ...
                          {g.rows.map((b) => (
                            <tr key={b.id}>
                              ...
                              <td>
                                <button
                                  type="button"
                                  title={b.status === 1 ? "Đang bật (Click để tắt)" : "Đang tắt (Click để bật)"}
                                  onClick={async () => {
                                    if (!window.confirm(`Bạn muốn ${b.status === 1 ? 'tắt' : 'bật'} lịch ca này?`)) return;
                                    try {
                                      await updateScheduleBlock(b.id, { ...b, status: b.status === 1 ? 0 : 1 });
                                      fetchBlocks(page);
                                    } catch (e) {
                                      alert('Lỗi cập nhật');
                                    }
                                  ...
                                  style={{
                                    ...
                                    borderRadius: 6, backgroundColor: b.status === 1 ? '#dcfce7' : '#f1f5f9'
                                  }}
                                >
          ...
          <form onSubmit={handleAddOverride} style={{ marginBottom: 24, padding: 16, background: '#f8fafc', borderRadius: 8 }}>
            ...
            <div className="form-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}>
              ...
              <div className="form-group">
                ...
                <select className="form-input" value={ovSessionType} onChange={(e) => setOvSessionType(e.target.value)}>
              ...
              <tbody>
                {overrides.map((o) => (
                  <tr key={o.id}>
                    ...
                    <td>
                      <span style={{ fontWeight: 600, color: o.session_type ? '#f59e0b' : '#ef4444' }}>
                        {o.session_type === 1 ? 'Sáng' : o.session_type === 2 ? 'Chiều' : 'Cả ngày'}
                      </span>
                    </td>
