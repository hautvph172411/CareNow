      ...
      <div className="appointments-page">
        {cancelledByPatientCount > 0 ? (
          <div style={{
            ...
            Có {cancelledByPatientCount} lịch vừa được bệnh nhân hủy. Vui lòng kiểm tra và cập nhật xử lý.
          </div>
        ) : null}
                ...
                <tbody>
                  {rows.map((row) => {
                    ...
                    <tr key={row.id} style={isPatientCancelled ? { background: '#fff7f7' } : undefined}>
                      <td>
                        <strong style={isPatientCancelled ? { textDecoration: 'line-through', color: '#9ca3af' } : undefined}>{row.booking_code || `#${row.id}`}</strong>
                      </td>
                      <td>
