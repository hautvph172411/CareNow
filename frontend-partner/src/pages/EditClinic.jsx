...
export default function EditClinic() {
  ...
  const validateForm = () => {
    const newErrors = {}
    if (!formData.name || !formData.name.toString().trim()) newErrors.name = 'Tên bác sĩ không được để trống'
    if (!formData.address || !formData.address.toString().trim()) newErrors.address = 'Địa chỉ không được để trống'
    setErrors(newErrors)
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!validateForm()) {
      alert("Vui lòng kiểm tra lại: Tên bác sĩ và Địa chỉ không được để trống!");
      return;
    }
    setIsLoading(true)
    try {
