...
export default function ScheduleWeekGrid({ 
  ...
  // Force adding the selected doctor to the grid so we can add blocks if they are currently empty
  if (selectedClinicId) {
    const selectedDoctor = clinics?.find(c => String(c.id) === String(selectedClinicId));
    if (selectedDoctor) {
      clinicMap.set(String(selectedClinicId), {
        clinicId: selectedDoctor.id,
        clinicName: selectedDoctor.name || `Bác sĩ #${selectedDoctor.id}`,
        days: { 1: [], 2: [], 3: [], 4: [], 5: [], 6: [], 0: [] }
      });
    } else {
      // Fallback if doctor is not in clinics list but we have their ID
      clinicMap.set(String(selectedClinicId), {
        clinicId: selectedClinicId,
        clinicName: `Bác sĩ #${selectedClinicId}`,
        days: { 1: [], 2: [], 3: [], 4: [], 5: [], 6: [], 0: [] }
      });
    }
  }

  // Initialize with all clinics that have blocks
  blocks.forEach(b => {
