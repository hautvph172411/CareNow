...
const getAllAppointments = async (req, res) => {
  try {
    ...
    const result = await service.getAllAppointments(query);
...
const getAppointmentById = async (req, res) => {
  try {
    ...
    const appt = await service.getAppointmentById(req.params.id, patientId, isAdmin, partnerId);
...
const updateAppointmentStatus = async (req, res) => {
  try {
    ...
    const partnerId = req.user?.partner_id ?? null;
    const appt = await service.updateAppointmentStatus(req.params.id, status, admin_notes, partnerId);
...
const updateAppointment = async (req, res) => {
  try {
    const partnerId = req.user?.partner_id ?? null;
    const appt = await service.updateAppointment(req.params.id, req.body, partnerId);
    return res.status(200).json({ message: 'Cập nhật thành công', data: appt });
  } catch (err) {
    return handleError(res, err);
  }
