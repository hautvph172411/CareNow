...
exports.getAll = async (query = {}) => {
  ...
  if (query.created_date) {
    q += ` AND a.created_at::date = $${idx++}`;
    values.push(query.created_date);
  }
  ...
  if (query.partner_id) {
    q += ` AND (
      cp.partner_id = $${idx} OR 
      EXISTS (
        SELECT 1 FROM tbl_clinic_place tcp 
        WHERE tcp.id::text = ANY(STRING_TO_ARRAY(COALESCE(c.place_ids, ''), ',')) 
          AND tcp.partner_id = $${idx}
      )
    )`;
