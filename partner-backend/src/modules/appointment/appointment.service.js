...
/* ── Chi tiết 1 lịch hẹn ─────────────────────────────────────────────────── */
const getAppointmentById = async (id, requesterId, isAdmin, partnerId = null) => {
  const appt = await repository.findById(id);
  if (!appt) throw new Error('NOT_FOUND');

  if (partnerId) {
    const isPartnerValid = (String(appt.place_partner_id) === String(partnerId)) || 
                           (appt.clinic_partner_ids && appt.clinic_partner_ids.split(',').includes(String(partnerId)));
    if (!isPartnerValid) throw new Error('FORBIDDEN');
  } else if (!isAdmin && appt.patient_id !== requesterId) {
...
const updateAppointmentStatus = async (id, status, adminNotes, partnerId = null) => {
...
const updateAppointment = async (id, payload = {}, partnerId = null) => {
  const appt = await repository.findById(id);
  if (!appt) throw new Error('NOT_FOUND');
