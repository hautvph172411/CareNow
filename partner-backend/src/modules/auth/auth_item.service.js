...
exports.deleteItem = async (name) => {
    ...
    // Nếu là role (type=1), kiểm tra xem có user nào đang được gán không
    if (exists.type === 1) {
        const assignCount = await repo.countAssignments(name);
        if (assignCount > 0) {
            const err = new Error(`Không thể xóa vai trò đang được gán cho ${assignCount} người dùng. Hãy gỡ gán trước.`);
            err.status = 400;
            throw err;
        }
    }
