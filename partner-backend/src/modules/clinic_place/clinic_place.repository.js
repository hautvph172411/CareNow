...
exports.getAll = async (query = {}) => {
  let q = `SELECT c.*, p.name AS partner_name, p.short_name AS partner_short_name 
           FROM tbl_clinic_place c 
           LEFT JOIN tbl_partner p ON c.partner_id = p.id 
           WHERE 1=1`;
  const values = [];
  let idx = 1;

  if (query.keyword) {
    q += ` AND (c.name ILIKE $${idx} OR c.address ILIKE $${idx} OR c.short_name ILIKE $${idx} OR p.name ILIKE $${idx})`;
  ...
  if (query.province_id) {
    q += ` AND c.province_id = $${idx}`;
    values.push(query.province_id);
    idx++;
  }
  if (query.partner_id !== undefined && query.partner_id !== '') {
    q += ` AND c.partner_id = $${idx}`;
    values.push(query.partner_id);
    idx++;
  }

  q += ' ORDER BY c.rank DESC NULLS LAST, c.created_at ASC NULLS LAST, c.order ASC NULLS LAST, c.id ASC';
