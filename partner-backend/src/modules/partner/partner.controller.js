...
exports.deletePartner = async (req, res) => {
  try {
    ...
    if (err.message === 'PARTNER_HAS_CLINIC_PLACES') {
      return res.status(400).json({ success: false, message: 'Không thể xóa đối tác vì đang có cơ sở y tế (nơi khám) liên kết!' });
    }
    if (err.message === 'PARTNER_HAS_USERS') {
      return res.status(400).json({ success: false, message: 'Không thể xóa đối tác vì đang có tài khoản nhân viên/quản lý liên kết!' });
    }
    res.status(500).json({ success: false, message: err.message });
  }
};
