...
exports.hasClinicPlaces = async (id) => {
  const result = await pool.query('SELECT COUNT(*) as count FROM tbl_clinic_place WHERE partner_id = $1', [id]);
  return parseInt(result.rows[0].count, 10) > 0;
};

exports.hasUsers = async (id) => {
  const result = await pool.query('SELECT COUNT(*) as count FROM tbl_user WHERE partner_id = $1 AND status != -1', [id]);
  return parseInt(result.rows[0].count, 10) > 0;
};
