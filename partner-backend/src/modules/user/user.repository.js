...
exports.createUser = async (user) => {
  ...
  const query = `
    ...
    VALUES (${placeholders})
    RETURNING id, username, email, phone, display_name, role, status, partner_id, partner_role, created_at
  `;
...
exports.getAll = async (query = {}) => {
  let q = 'SELECT id, username, email, phone, display_name, role, status, partner_id, partner_role, created_at FROM tbl_user WHERE status != -1';
  const values = [];
  let idx = 1;
